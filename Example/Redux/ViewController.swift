...
class ViewController: UIViewController, StoreDelegate {
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        self.connect(Store.appStore, keys: [ kAppStateKeyTodoList ], delegate: self)
    ...
    func storeDidUpdateState(lastState: ReduxAppState) {
        let lastTodoListState = lastState.get(kAppStateKeyTodoList) as! TodoListState
...
extension ViewController: UITableViewDataSource {
    ...
    internal func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        let cell = UITableViewCell(style: UITableViewCellStyle.Subtitle, reuseIdentifier: "TodoListItem.cell")
